%run_Met_Has_Longstep2014mar14.m
%Trimmed from 2014mar13, update to m_pakke_2014mar13
%news: 
% fs.RelErrorObs may no be a vector, one relative error per nucleide or nucleide ratio. 
% 
%Developed from run_MetHas_longstep2.m
%Purpose: Run Met_Has_Longstep2, which includes linear inspiration proposer
%run_Met_Has_Longstep2 includes also burn-in
...
%     fs.Nucleides = {'26Al/10Be','14C'}; %We may switch nucleides on and off
%     fs.RelErrorObs = 0.02;%0.02; %0.02 means 2% observational error
    fs.RelErrorObs = 0.01*[2;4;10;1];%0.02; %0.02 means 2% observational error
%     fs.zobs = [0]; %Depths where nucleides are observed
    ...
    if strcmp(fs.dobsMode,'ObservedData')
      fs.d_obs = [];
      fs.d_obs = repmat([1.23e7;1.77e8;1.32e9;1.55e10]',length(fs.zobs),1); %<<<<<< put in values as best you can
      fs.d_obs = fs.d_obs(:);
    end
    ...
    fs.CycleMode = 'FixedQuaternary'; %'FixedC','FixedQuaternary','FixedTimes'
    switch fs.CycleMode
      case 'FixedC'
      ...
      case 'FixedTimes'
        fs.tGlas = NaN; %load or compute fixed times of glaciation starts
        fs.tInt = NaN; %load or compute fixed times of glaciation ends
    end
    ...
    fs.tDeglaminmax   = [5e3,15e3]; %8000 to 10000 yr Holocene
...
for iwalk=1:fixed_stuff.Nwalkers
    ...
    isBurnIn=1; 
    [S.msBurnIn,S.acceptsBurnIn,S.QsBurnIn,S.QdsBurnIn,S.lump_MetHas_BurnIn]=MetHasLongstep3(...
    ...
    [S.ms,S.accepts,S.Qs,S.Qds,S.lump_MetHas]=MetHasLongstep3(...
