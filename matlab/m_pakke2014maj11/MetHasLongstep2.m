...
% d_obs: Observed data
% C_obs: Covariance matrix of measurement uncertainty
% m_prior: Prior model
% C_prior: Covariance matrix of deviations from prior model
% fixed-stuff: A variable, possible a structure which defines non-variable
% parameters for function g
% N_run: Number of iterations after burn-in, using linearized inspiration
% N_skip: We store only one out of N_skip states.
% proposer_type: 1 = isotropic unit coordinate variances:
%                   u = randn(size(m_start))
%               2 = box-shaped coordinate variances:
%                   u = sqrt(12)*(rand(size(m_start))-0.5)
%               3 = prior proposer:
%                   u = chol(C_prior)*randn(size(m))
%               4 = proposer defined in fixed_stuff
%                   u = chol(fixed_stuff.C_prop)*randn(size(m))
%               5 = proposer defined by external function
%                   u = prop_external;
% seed: If seed==0
%                  %we do not set seed for rand and randn,
%               else
%                  rand('seed',seed);
%                  randn('seed',seed);
%               end
% k: Scale factor to multiply onto step defined by proposer_type:
%    m_proposed = m_current + k*u;
%OUTPUTS:
...
function [ms,accepts,Qs,Qds,lump]=MetHasLongstep(...
  m_start,d_obs,seed,isBurnIn,fixed_stuff);%,k)
if isBurnIn
