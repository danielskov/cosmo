...
% consts.Natoms10 -  atom number density of the target atom (atoms/g)
% consts.k_neg10 - summary cross-section for negative muon capture (atoms/muon)
% consts.sigma190_10 - 190 GeV x-section for fast muon production (cm2)
%
...
%sigma0 = consts.sigma190_10./(190.^aalpha);
sigma0 = consts.sigma190_26./(190.^aalpha);

% fast muon production

%P_fast = phi.*Beta.*(Ebar.^aalpha).*sigma0.*consts.Natoms10;
P_fast = phi.*Beta.*(Ebar.^aalpha).*sigma0.*consts.Natoms26;

% negative muon capture

%P_neg = R.*consts.k_neg10;
P_neg = R.*consts.k_neg26;
