...
NudgeTest = 1; % 0 if you just want to use the default, nudge_factor = 1e-10
...
axh(1)=subplot(3,1,1)
if NudgeTest
  nudge_factor = 1e-3; %<<---- play with this. 1e-10 is a good value in practice
  [h1,h2,h3,i_cross]=fill_red_blue(xs,ys,colpos,colneg,midvalue,nudge_factor);
  title(['Nudge_factor = ',num2str(nudge_factor)],'interpreter','none')
  plot(xs,ys,'.g')
else
  [h1,h2,h3,i_cross]=fill_red_blue(xs,ys,colpos,colneg,midvalue); %use default nudge_factor
end
ylabel('\delta18O')
axis tight
